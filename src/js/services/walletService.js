...
angular.module('copayApp.services').factory('walletService', function ($log, $timeout, lodash, trezor, ledger, intelTEE, storageService, configService, rateService, uxLanguage, $filter, gettextCatalog, bwcError, $ionicPopup, fingerprintService, ongoingProcess, gettext, $rootScope, txFormatService, $ionicModal, $state, bwcService, bitcore, popupService, firebaseEventsService, payproService, profileService) {
  ...
  var LOW_AMOUNT_RATIO = 0.15; // Ratio of "many utxos" warning in total balance (fee/amount)
  ...
  var _signWithLedger = function _signWithLedger(wallet, txp, cb) {
  ...
  var _signWithTrezor = function _signWithTrezor(wallet, txp, cb) {
  ...
  var _signWithIntelTEE = function _signWithIntelTEE(wallet, txp, cb) {
  ...
  root.getStatus = function (wallet, opts, cb) {
    ...
    function processPendingTxps(status) {
      ...
      */
      ...
      lodash.each(txps, function (tx) {
        tx = txFormatService.processTx(wallet.coin, tx); // no future transactions...
    ...
    function get(cb) {
      wallet.getStatus({
        ...
        if (err) {
          if (_instanceof(err, errors.NOT_AUTHORIZED)) {
    ...
    function cacheBalance(wallet, balance) {
      ...
      var cache = wallet.cachedStatus; // Address with Balance

      cache.balanceByAddress = balance.byAddress; // Total wallet balance is same regardless of 'spend unconfirmed funds' setting.

      cache.totalBalanceSat = balance.totalAmount; // Spend unconfirmed funds
      ...
      cache.satToUnit = 1 / cache.unitToSatoshi; //STR
      ...
      cache.alternativeIsoCode = config.settings.alternativeIsoCode; // Check address

      root.isAddressUsed(wallet, balance.byAddress, function (err, used) {
        if (used) {
          $log.debug('Address used. Creating new'); // Force new address
    ...
    function _getStatus(initStatusHash, tries, cb) {
      ...
      get(function (err, status) {
        ...
        if (opts.untilItChanges && initStatusHash == currentStatusHash && tries < root.WALLET_STATUS_MAX_TRIES && walletId == wallet.credentials.walletId) {
  ...
  var getSavedTxs = function getSavedTxs(walletId, cb) {
  ...
  var getTxsFromServer = function getTxsFromServer(wallet, skip, endingTxid, limit, cb) {
    var res = [];
    wallet.getTxHistory({
      skip: skip,
      limit: limit,
      ...
      if (err) return cb(err);
      if (!txsFromServer.length) return cb(null, []);
      return cb(null, txsFromServer);
    });
  };

  var removeAndMarkSoftConfirmedTx = function removeAndMarkSoftConfirmedTx(txs) {
  ...
  var processNewTxs = function processNewTxs(wallet, txs) {
  ...
  var updateLocalTxHistory = function updateLocalTxHistory(wallet, opts, cb) {
    var FIRST_LIMIT = 5;
    var LIMIT = 50;
    ...
    getSavedTxs(walletId, function (err, txsFromLocal) {
      ...
      console.log('pagination Hard confirmed TXs. Got:' + confirmedTxs.length + '/' + txsFromLocal.length); // First update
      ...
      function getNewTxs(newTxs, skip, next) {
        ...
        getTxsFromServer(wallet, skip, endingTxid, requestLimit, function (err, res) {
          if (err) {
            ...
            if (_instanceof(err, errors.CONNECTION_ERROR) || err.message && err.message.match(/5../)) {
          ...
          console.log('pagination Result count: ' + res.length); // Check if new txs are founds, if yes, lets investigate in the 50 next
          ...
          var shouldContinue = newDiscoveredTxs.length > 0; // If no new tx, no need to check
          ...
          console.log('pagination Syncing TXs. Got:' + newTxs.length + ' Skip:' + skip, ' EndingTxid:', endingTxid, ' Continue:', shouldContinue); // TODO Dirty <HACK>
      ...
      function createReceivedEvents(newTxs) {
        storageService.getReceivedTransactions(walletId, function (err, txIds) {
          ...
          lodash.each(notInStorage, function (t) {
            firebaseEventsService.logEvent('received_bitcoin', {
              coin: wallet.coin
            });
          });
      ...
      if (opts.getMoreTxs) {
        ...
        getNewTxs([], skipped, function (err, txs) {
          ...
          updateNotes(function () {
            ...
            $log.debug('Tx History synced. Total Txs: ' + newHistory.length); // Final update
  ...
  root.getTxHistory = function (wallet, opts, cb) {
    opts = opts || {};
    if (!wallet.isComplete()) return cb(); // var historyIsCached = wallet.completeHistory && wallet.completeHistory.isValid;
  ...
  root.createTx = function (wallet, txp, cb) {
    if (lodash.isEmpty(txp) || lodash.isEmpty(wallet)) return cb('MISSING_PARAMETER'); // If we are spending from our dedicated CashShuffle Spend-Only wallet,
  ...
  root.updateRemotePreferences = function (clients, prefs, cb) {
    ...
    var walletSettings = config.wallet.settings; //prefs.email  (may come from arguments)

    prefs.email = config.emailNotifications.email;
    prefs.language = uxLanguage.getCurrentLanguage(); // prefs.unit = walletSettings.unitCode; // TODO: remove, not used
  ...
  var createAddress = function createAddress(wallet, cb, ignoreMaxGap) {
    ...
    wallet.createAddress(createParams, function onWalletCreatedAddress(err, addr) {
      if (err) {
        ...
        } else if (_instanceof(err, errors.MAIN_ADDRESS_GAP_REACHED) || err.message && err.message == 'MAIN_ADDRESS_GAP_REACHED') {
          $log.warn(err);
          prefix = null;
  ...
  root.getLowUtxos = function (wallet, levels, cb) {
    wallet.getUtxos({
      ...
      var balance = lodash.sum(resp, 'satoshis'); // for 2 outputs
  ...
  var askPassword = function askPassword(name, title, cb) {
    var opts = {
      inputType: 'password',
  ...
  root.publishAndSign = function (wallet, txp, cb, customStatusHandler) {
    var publishFn = root.publishTx; // Already published?

    if (txp.status == 'pending') {
      publishFn = function publishFn(wallet, txp, cb) {
        return cb(null, txp);
      };
    }
      ...
      publishFn(wallet, txp, function (err, publishedTxp) {
        ...
        root.signTx(wallet, publishedTxp, password, function (err, signedTxp) {
          ...
          if (err) {
            ...
            var msg = err && err.message ? err.message : gettextCatalog.getString('The payment was created but could not be completed. Please try again from home screen');
            $rootScope.$emit('Local/TxAction', wallet.id);
            return cb(msg);
          }
          ...
          if (signedTxp.status == 'accepted') {
            ...
            var handleBroadcastTx = function handleBroadcastTx(err, broadcastedTxp) {
  ...
  root.getProtocolHandler = function (wallet) {
    if (wallet.coin == 'bch') return 'bitcoincash';else return 'bitcoin';
  };
  ...
  root.copyCopayers = function (wallet, newWallet, cb) {
    ...
    lodash.each(c.publicKeyRing, function (item) {
      ...
      newWallet._doJoinWallet(newWallet.credentials.walletId, walletPrivKey, item.xPubKey, item.requestPubKey, name, {
        ...
        if (err && !_instanceof(err, errors.COPAYER_IN_WALLET)) {
          mainErr = err;
        }
